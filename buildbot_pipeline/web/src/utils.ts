...
export function fmtDuration(item: {complete: boolean, complete_at: number, started_at: number}): string {
    var dur
    if (item.complete) {
...
export function buildLink(build: {builderid: number, number: number}) {
    return {name: 'build', params: {builderid: build.builderid, number: build.number}}
}

export function resultClass(obj: {results: number}, no_pulse: boolean = false): string {
    var num = obj.results;
    var add = '';
    if (num == null) {
        num = 99;
